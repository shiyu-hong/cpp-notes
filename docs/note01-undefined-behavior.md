...
在 C++ 中，**空指针（Null Pointer）**是一个特殊指针值，表示指针不指向任何有效的对象或内存地址。
它确保指针处于“未指向任何内容”状态的明确标识。
若对其进行解引用（访问或修改内存），会导致未定义行为，可能引发程序崩溃、数据损坏或难以调试的逻辑错误。
...
在 C++ 中，**野指针（Dangling Pointer）**是指向已释放或无效内存地址的指针。
这些指针不再合法，但依然保留原来的地址值。
若对其进行解引用（访问或修改内存），会导致未定义行为，可能引发程序崩溃、数据损坏或难以调试的逻辑错误。
